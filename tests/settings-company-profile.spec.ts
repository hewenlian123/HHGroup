...
const BRANDING_FULL =
  process.env.E2E_BRANDING_FULL === "1" || process.env.E2E_BRANDING_FULL === "true";
...
test.describe("Settings → Company Profile", () => {
  ...
  test("invalid email shows error and does not show success toast", async ({ page }) => {
    ...
    await expect(
      page
        .locator('[role="status"]')
        .filter({ hasText: /Invalid email|Enter a valid email address/i })
  ...
  test.describe.serial("Company profile mutations (shared row)", () => {
    test("company name (org_name) changes from HH Group and persists after reload", async ({
      ...
      const matchesSavePayload = (req: import("@playwright/test").Request): boolean => {
        if (!req.url().includes("/api/settings/company-profile") || req.method() !== "POST")
          return false;
      ...
      await waitForCompanyProfileReady(page);
      await expect(page.getByTestId("company-input-org_name")).toHaveValue(newName, {
        timeout: 45_000,
      });
    });
    ...
    test("logo upload rejects non-image file", async ({ page }) => {
      ...
      try {
        ...
        await expect(
          page.locator('[role="status"]').filter({ hasText: "Upload failed" })
        ).toBeVisible({ timeout: 15_000 });
        await expect(
          page.locator('[role="status"]').filter({ hasText: /image file/i })
        ).toBeVisible();
      } finally {
    ...
    test("logo: upload small PNG shows preview; replace with SVG; remove clears preview", async ({
      page,
    }) => {
      await page.goto(`${BASE}/settings/company`);
      await page.waitForLoadState("domcontentloaded");
      ...
      try {
        ...
        const outcome = page
          .locator('[role="status"]')
          .filter({ hasText: /Logo uploaded|Upload failed/ });
        ...
        if (label.includes("Upload failed")) {
          if (BRANDING_FULL) {
            throw new Error(
              "Logo upload failed but E2E_BRANDING_FULL=1 (expect working `branding` bucket + RLS)."
            );
          }
          test.skip(
            true,
            "Logo upload blocked (configure `branding` storage + RLS, or set E2E_BRANDING_FULL=1 to fail hard)."
          );
        }
        ...
        await expect(
          page.locator('[role="status"]').filter({ hasText: "Logo uploaded" }).first()
        ).toBeVisible({
          timeout: 30_000,
        });
        await expect(page.getByRole("img", { name: "Company logo" })).toBeVisible();
        ...
        await expect(
          page.locator('[role="status"]').filter({ hasText: "Logo removed" }).first()
        ).toBeVisible({
          timeout: 30_000,
        });
    ...
    test("save failure shows error toast and button recovers", async ({ page }) => {
      ...
      try {
        ...
        await saveBtn.click();
        await expect(
          page.locator('[role="status"]').filter({ hasText: "Save failed" })
        ).toBeVisible({ timeout: 30_000 });
        await expect(saveBtn).toContainText("Save Profile", { timeout: 15_000 });
      } finally {
