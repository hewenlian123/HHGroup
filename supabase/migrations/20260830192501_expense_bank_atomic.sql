...
alter table public.expenses
  ...
  add column if not exists idempotency_group_index bigint,
  add column if not exists idempotency_group_count bigint,
  add column if not exists idempotency_completed_at timestamptz;
...
do $$
begin
  if to_regclass('public.idx_expenses_atomic_idempotency_group') is not null
    and not pg_temp.financial_unique_index_ready(
      to_regclass('public.idx_expenses_atomic_idempotency_group'),
      to_regclass('public.expenses'),
      array['idempotency_key', 'idempotency_group_index'],
      'idempotency_keyisnotnull'
    )
  then
    execute 'drop index public.idx_expenses_atomic_idempotency_group';
  end if;
end;
$$;

create unique index concurrently if not exists idx_expenses_atomic_idempotency_group
  on public.expenses (idempotency_key, idempotency_group_index)
...
begin
  if not pg_temp.financial_unique_index_ready(
    ...
    raise exception 'Financial unique index idx_expenses_atomic_idempotency_group is not valid and ready.';
  end if;
end;
$$;
...
do $$
begin
  if to_regclass('public.idx_expenses_bank_transaction_source') is not null
    and not pg_temp.financial_unique_index_ready(
      to_regclass('public.idx_expenses_bank_transaction_source'),
      to_regclass('public.expenses'),
      array['source', 'source_id'],
      'source=''bank_transaction''andsource_idisnotnull'
    )
  then
    execute 'drop index public.idx_expenses_bank_transaction_source';
  end if;
end;
$$;

create unique index concurrently if not exists idx_expenses_bank_transaction_source
  on public.expenses (source, source_id)
  where source = 'bank_transaction' and source_id is not null;
...
begin
  if not pg_temp.financial_unique_index_ready(
    ...
    raise exception 'Financial unique index idx_expenses_bank_transaction_source is not valid and ready.';
  end if;
end;
$$;

alter table public.bank_transactions
  add column if not exists reconcile_idempotency_key text,
  add column if not exists reconcile_fingerprint text;

do $$
begin
  if to_regclass('public.idx_bank_transactions_reconcile_idempotency_key') is not null
    and not pg_temp.financial_unique_index_ready(
      to_regclass('public.idx_bank_transactions_reconcile_idempotency_key'),
      to_regclass('public.bank_transactions'),
      array['reconcile_idempotency_key'],
      'reconcile_idempotency_keyisnotnull'
    )
  then
    execute 'drop index public.idx_bank_transactions_reconcile_idempotency_key';
  end if;
end;
$$;

create unique index concurrently if not exists idx_bank_transactions_reconcile_idempotency_key
  on public.bank_transactions (reconcile_idempotency_key)
  where reconcile_idempotency_key is not null;
...
begin
  if not pg_temp.financial_unique_index_ready(
    ...
    raise exception 'Financial unique index idx_bank_transactions_reconcile_idempotency_key is not valid and ready.';
  end if;
end;
$$;
...
comment on column public.expenses.idempotency_key is
  'Server-validated request key for atomic expense creation. One request may create one expense per project group.';
comment on column public.expenses.idempotency_fingerprint is
  'SHA-256 of the canonical JSONB create payload; changed-payload replays are rejected.';
comment on column public.expenses.idempotency_completed_at is
  'Set only after every expense header, line, deduction, and metadata write in the create intent succeeds.';
comment on column public.bank_transactions.reconcile_idempotency_key is
  'Server-enforced exactly-once key for Expense reconciliation.';
comment on column public.bank_transactions.reconcile_fingerprint is
  'SHA-256 of the canonical bank Expense reconciliation payload.';
...
begin
  ...
  if v_operation = 'add' then
    insert into public.expense_lines (
      expense_id, project_id, cost_code, description, qty, unit_cost, amount, total
    ) values (
      p_expense_id,
    ...
    if coalesce(p_preserve_last_line, false) and v_line_count <= 1 then
      update public.expense_lines el
      set project_id = null, cost_code = null, description = null, amount = 0, total = 0, unit_cost = 0
      where el.id = v_line_id and el.expense_id = p_expense_id;
    else
