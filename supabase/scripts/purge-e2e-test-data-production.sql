-- ═══════════════════════════════════════════════════════════════════════════
-- Purge E2E / Playwright / seed.sql residue from a database (e.g. production recovery).
-- Run in Supabase SQL Editor as postgres (RLS bypassed for table owner / service role context).
--
-- Prerequisite: run audit-e2e-residuals-production.sql and review counts.
--
-- Optional created_at window: add AND created_at >= '...' AND created_at < '...' inside
-- relevant DELETEs if you only want rows from known test windows.
--
-- Heuristics: E2E / Seed / [E2E] in labels; vendor/reference E2E-PM-INLINE-%, E2E-Pay-,
--   fixed seed UUIDs (1111…, 2222…, 3333…, 4444…, 6666…661).
--
-- Also deletes: ap_bill_payments (children of matching ap_bills), bank_transactions rows
-- that clearly match E2E text (after unlinking from expenses).
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

-- ─── 0) Expense IDs touched by E2E (single definition for unlink + children) ─
-- Includes Playwright vendor prefixes, inline payment-method spec, and broad E2E/Seed vendor text.

-- ─── 1) Bank: unlink (if table exists); final delete at end of script ─────────
DO $blk$
BEGIN
...
-- Same project list as section 8 (projects delete): rows may not match person/notes heuristics above
-- but still block FK project_commissions_project_id_fkey.
DELETE FROM public.commission_payments cp
WHERE EXISTS (
  SELECT 1
