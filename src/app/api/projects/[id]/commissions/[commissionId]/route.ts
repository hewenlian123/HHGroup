...
export async function PATCH(
  ...
  if (!commissionId) return NextResponse.json({ ok: false, message: "Missing commission id" }, { status: 400 });
  try {
    ...
    if (body.calculation_mode !== undefined) updates.calculation_mode = MODES.includes(body.calculation_mode) ? body.calculation_mode : "Auto";
    if (body.rate !== undefined) updates.rate = Math.max(0, Number(body.rate) || 0);
    if (body.base_amount !== undefined) updates.base_amount = Math.max(0, Number(body.base_amount) || 0);
    if (body.commission_amount !== undefined) updates.commission_amount = Math.max(0, Number(body.commission_amount) || 0);
    if (body.status !== undefined) updates.status = STATUSES.includes(body.status) ? body.status : "Pending";
    if (body.notes !== undefined) updates.notes = body.notes != null ? String(body.notes).trim() || null : null;
    const commission = await updateCommission(commissionId, updates as Parameters<typeof updateCommission>[1]);
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ ok: false, message: "Commission not found" }, { status: 404 });
