...
import { parseCloseoutPunchInput } from "@/lib/project-closeout-validation";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  let input: ReturnType<typeof parseCloseoutPunchInput>;
  try {
    input = parseCloseoutPunchInput(await req.json());
  } catch {
  ...
  try {
    await upsertCloseoutPunch(id, input.value, admin);
