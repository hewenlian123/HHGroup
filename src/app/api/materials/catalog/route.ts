...
export async function PATCH(req: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ ok: false as const, message: "Material id is required." }, { status: 400 });
    }
    const material_name = String(body.material_name ?? "").trim();
    if (!material_name) {
      return NextResponse.json({ ok: false as const, message: "Material name is required." }, { status: 400 });
    }
    const updated = await updateMaterial(id, {
      category: String(body.category ?? "").trim() || "Uncategorized",
      material_name,
      supplier: body.supplier != null && String(body.supplier).trim() !== "" ? String(body.supplier).trim() : null,
      cost: body.cost !== "" && body.cost != null && Number.isFinite(Number(body.cost)) ? Number(body.cost) : null,
      photo_url: body.photo_url != null && String(body.photo_url).trim() !== "" ? String(body.photo_url).trim() : null,
      description: body.description != null && String(body.description).trim() !== "" ? String(body.description).trim() : null,
    });
    if (!updated) {
      return NextResponse.json({ ok: false as const, message: "Failed to update material." }, { status: 500 });
    }
