import { NextResponse } from "next/server";
import { getServerSupabaseAdmin } from "@/lib/supabase-server";

const BUCKET = "material-images";
const PREFIX = "selections";

export async function POST(req: Request) {
  const supabase = getServerSupabaseAdmin();
  if (!supabase) {
    return NextResponse.json(
      { ok: false as const, message: "Supabase not configured." },
      { status: 500 }
    );
  }

  try {
    const formData = await req.formData();
    const file = formData.get("file") as File | null;
    if (!file?.size) {
      return NextResponse.json(
        { ok: false as const, message: "No file provided." },
        { status: 400 }
      );
    }
    if (!file.type.startsWith("image/")) {
      return NextResponse.json(
        { ok: false as const, message: "Only image files are allowed." },
        { status: 415 }
      );
    }

    const ext = file.name.split(".").pop()?.toLowerCase() || "jpg";
    const path = `${PREFIX}/${crypto.randomUUID()}.${ext}`;
    const { error } = await supabase.storage.from(BUCKET).upload(path, file, {
      contentType: file.type || "image/jpeg",
      upsert: false,
    });
    if (error) {
      return NextResponse.json({ ok: false as const, message: error.message }, { status: 500 });
    }
    return NextResponse.json({
      ok: true as const,
      path,
      imageUrl: `/api/materials/photo?path=${encodeURIComponent(path)}`,
    });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Upload failed.";
    return NextResponse.json({ ok: false as const, message }, { status: 500 });
  }
}
