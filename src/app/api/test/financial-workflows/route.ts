...
export async function POST(req: Request) {
  ...
  if (run("labor_workflow")) try {
    ...
    if (!workerId || !projectId) {
      ...
      if (entries.length === 0) {
        ...
        const reduced = balanceAfter < balanceBefore || Math.abs(balanceAfter - (balanceBefore - payAmount)) < 0.01;
        if (!reduced && balanceBefore > 0) steps.push("balance reduced check: " + (reduced ? "yes" : "no"));
  ...
  if (run("reimbursement_workflow")) try {
    ...
    if (!workerId) {
      ...
      if (!reimbursementCreated) {
        tests.push({ name: "reimbursement_workflow", ok: false, steps: ["Approve did not create reimbursement"] });
      } else {
        ...
        if (!payRes.ok) {
          tests.push({ name: "reimbursement_workflow", ok: false, steps: [...steps, "Mark paid failed: " + (payData.message ?? payRes.statusText)] });
        } else {
          ...
          const noDuplicate = payRes2.ok && (payData2.expenseId === undefined || payData2.expenseId === firstExpenseId);
          ...
          const stillPending = list.filter((r) => r.status === "pending").some((r) => r.id === reimbId);
  ...
  if (run("worker_invoice_workflow")) try {
    ...
    if (!workerId || !projectId) {
      ...
      const profitBefore = await getCanonicalProjectProfit(projectId).catch(() => ({ expenseCost: 0 }));
      ...
      const profitAfter = await getCanonicalProjectProfit(projectId).catch(() => ({ expenseCost: 0 }));
      const costUpdated = (profitAfter?.expenseCost ?? 0) >= (profitBefore?.expenseCost ?? 0) + 99;
  ...
  if (run("expense_workflow")) try {
    ...
    if (!projectId) {
      ...
      const profitBefore = await getCanonicalProjectProfit(projectId).catch(() => ({ actualCost: 0, profit: 0 }));
      const expense = await createExpense({
        date: new Date().toISOString().slice(0, 10),
      ...
      const profitAfter = await getCanonicalProjectProfit(projectId).catch(() => ({ actualCost: 0, profit: 0 }));
      const spentIncreased = (profitAfter?.actualCost ?? 0) >= (profitBefore?.actualCost ?? 0) + 74;
      ...
      tests.push({
        ...
        ok: !!expense?.id && (spentIncreased || (profitBefore?.actualCost ?? 0) === (profitAfter?.actualCost ?? 0)) && profitRecalc,
        steps,
      });
    }
  } catch (e) {
  ...
  if (run("invoice_payment_workflow")) try {
    ...
    if (!projectId) {
      ...
      const statusUpdated = withDerived?.computedStatus === "Partial" || withDerived?.computedStatus === "Paid";
      steps.push(statusUpdated ? "invoice status updated" : "status: " + (withDerived?.computedStatus ?? "?"));
      const outstandingReduced = withDerived != null && withDerived.paidTotal >= 99 && withDerived.balanceDue < 201;
