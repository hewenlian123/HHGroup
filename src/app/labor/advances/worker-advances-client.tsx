...
function AdvanceStatusChip({ status }: { status: AdvanceRow["status"] }) {
  if (status === "pending") return <span className={statusChipClass("warning")}>Pending</span>;
  if (status === "deducted") return <span className={statusChipClass("success")}>Deducted</span>;
  return <span className={statusChipClass("neutral")}>Cancelled</span>;
}
