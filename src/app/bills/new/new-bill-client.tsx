    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="max-w-xl space-y-4 py-4">
        ...
        <div>
          ...
          <Input type="number" step="0.01" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-1 h-9" placeholder="0.00" required />
        </div>
        <div>
        ...
        <div className="flex gap-2 pt-2">
          <Button type="submit" size="sm" disabled={submitting}>{submitting ? "Creating…" : "Create bill"}</Button>
          <Button type="button" variant="outline" size="sm" asChild><Link href="/bills">Cancel</Link></Button>
        </div>
      </form>
    </>
  );
