...
export function EstimatePaymentSchedule(props: {
  ...
  const { estimateId, paymentSchedule, estimateTotal, isLocked, addPaymentMilestoneAction, deletePaymentMilestoneAction } = props;
  const [scheduleOpen, setScheduleOpen] = React.useState(false);

  const totalScheduled = paymentSchedule.reduce((sum, item) => sum + paymentMilestoneAmount(item, estimateTotal), 0);
      ...
      <div className="p-0">
        ...
        <div className="px-4 py-3 flex flex-wrap items-center gap-6 text-sm border-b border-zinc-200/60 dark:border-border">
          <span className="text-muted-foreground">
            Estimate total <span className="font-semibold text-foreground tabular-nums">${fmt(estimateTotal)}</span>
          </span>
          <span className="text-muted-foreground">
            Scheduled <span className={cn("font-semibold tabular-nums", totalsMatch ? "text-emerald-600 dark:text-emerald-500" : "text-foreground")}>${fmt(totalScheduled)}</span>
          </span>
          <span className="text-muted-foreground">
            Remaining <span className={cn("font-semibold tabular-nums", remaining === 0 ? "text-emerald-600 dark:text-emerald-500" : "text-foreground")}>${fmt(remaining)}</span>
          </span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200/60 dark:border-border bg-muted/30">
                <th className="text-left py-2.5 px-4 font-medium text-muted-foreground">Payment Name</th>
                <th className="text-right py-2.5 px-4 font-medium text-muted-foreground tabular-nums">Amount</th>
                <th className="text-left py-2.5 px-4 font-medium text-muted-foreground">Payment Terms</th>
                <th className="text-left py-2.5 px-4 font-medium text-muted-foreground">Due Date</th>
            ...
            <tbody>
              {paymentSchedule.length === 0 ? (
                ...
                paymentSchedule.map((item) => {
                  ...
                  const dueDateDisplay = item.dueDate ? new Date(item.dueDate).toLocaleDateString(undefined, { dateStyle: "short" }) : "—";
                  return (
                    <tr key={item.id} className="border-b border-zinc-100/50 dark:border-border/30 hover:bg-muted/20 transition-colors">
                      <td className="py-2.5 px-4 font-medium text-foreground">{item.title || "—"}</td>
                      <td className="py-2.5 px-4 text-right tabular-nums font-medium text-foreground">
                        ${fmt(amount)}
                        {item.amountType === "percent" && <span className="ml-2 text-xs text-muted-foreground">({item.value}%)</span>}
                      </td>
                      <td className="py-2.5 px-4 text-muted-foreground">{item.dueRule || "—"}</td>
                      <td className="py-2.5 px-4 text-muted-foreground tabular-nums">{dueDateDisplay}</td>
                      {!isLocked ? (
                        <td className="py-2 px-2 align-middle">
                          <form action={deletePaymentMilestoneAction} className="flex justify-end">
                            <input type="hidden" name="estimateId" value={estimateId} />
                            <input type="hidden" name="itemId" value={item.id} />
                            <Button type="submit" variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10" aria-label="Delete">
              ...
              <form action={addPaymentMilestoneAction} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <select name="amountType" defaultValue="fixed" className="h-9 rounded-md border border-input bg-background px-2 text-sm flex-1">
                      <option value="fixed">Fixed</option>
                      <option value="percent">Percent</option>
                    </select>
                    <Input name="value" type="number" step="0.01" min={0} placeholder="30" className="h-9 w-28" required />
                  </div>
                </div>
                <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  <Button type="button" variant="outline" className="rounded-md" onClick={() => setScheduleOpen(false)}>
                    Cancel
                  </Button>
                </div>
              </form>
