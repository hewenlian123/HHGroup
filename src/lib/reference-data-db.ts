...
const DEFAULT_VENDORS = ["Home Depot", "Materials Co.", "Steel Supply Inc.", "City Permit Office", "Equipment Rentals"];
const DEFAULT_PAYMENT_METHODS = ["ACH", "Card", "Cash", "Check", "Wire", "Zelle"];

async function ensureExpenseCategoriesExist(): Promise<void> {
  const c = client();
  const { data: existing, error } = await c.from("categories").select("name").eq("type", "expense").limit(1);
...
export async function getExpenseCategories(includeDisabled = false): Promise<string[]> {
  ...
  const { data: rows, error } = await c.from("categories").select("name, status").eq("type", "expense").order("name");
  if (error) {
...
export async function addExpenseCategory(name: string): Promise<string> {
  ...
  if (!trimmed) return "";
  const c = client();
  const { data: existing } = await c.from("categories").select("name").eq("type", "expense").ilike("name", trimmed).maybeSingle();
  if (existing) return (existing as { name: string }).name;
  const { data: disabled } = await c.from("categories").select("id").eq("type", "expense").ilike("name", trimmed).eq("status", "inactive").maybeSingle();
  if (disabled) {
    await c.from("categories").update({ status: "active" }).eq("type", "expense").ilike("name", trimmed);
    return trimmed;
  }
...
export async function getCategoryUsageCount(name: string): Promise<number> {
  const c = client();
  const { count } = await c.from("expense_lines").select("id", { count: "exact", head: true }).eq("category", name);
  return count ?? 0;
}
...
export async function renameExpenseCategory(oldName: string, newName: string): Promise<boolean> {
  ...
  const { data: row } = await c.from("categories").select("id").eq("type", "expense").ilike("name", oldTrim).maybeSingle();
  if (!row) return false;
  await c.from("categories").update({ name: newTrim }).eq("id", (row as { id: string }).id);
...
export async function disableExpenseCategory(name: string): Promise<boolean> {
  const c = client();
  const { error } = await c.from("categories").update({ status: "inactive" }).eq("type", "expense").ilike("name", name.trim());
  return !error;
}

export async function enableExpenseCategory(name: string): Promise<boolean> {
  const c = client();
  const { error } = await c.from("categories").update({ status: "active" }).eq("type", "expense").ilike("name", name.trim());
  return !error;
}
...
export async function isExpenseCategoryDisabled(name: string): Promise<boolean> {
  const c = client();
  const { data: row } = await c.from("categories").select("status").eq("type", "expense").ilike("name", name.trim()).maybeSingle();
...
export async function getVendorUsageCount(name: string): Promise<number> {
  const c = client();
  const { count } = await c.from("expenses").select("id", { count: "exact", head: true }).ilike("vendor_name", name);
  return count ?? 0;
}

export async function renameVendor(oldName: string, newName: string): Promise<boolean> {
  const c = client();
  const { data: row } = await c.from("vendors").select("id").ilike("name", oldName.trim()).maybeSingle();
  if (!row) return false;
  await c.from("vendors").update({ name: newName.trim() }).eq("id", (row as { id: string }).id);
  await c.from("expenses").update({ vendor_name: newName.trim() }).ilike("vendor_name", oldName.trim());
  return true;
}

export async function disableVendor(name: string): Promise<boolean> {
  const c = client();
  const { error } = await c.from("vendors").update({ status: "inactive" }).ilike("name", name.trim());
  return !error;
}
...
export async function isVendorDisabled(name: string): Promise<boolean> {
  const c = client();
  const { data: row } = await c.from("vendors").select("status").ilike("name", name.trim()).maybeSingle();
  return (row as { status: string } | null)?.status === "inactive";
}
...
export async function getPaymentMethods(includeDisabled = false): Promise<string[]> {
  ...
  const { data: rows, error } = await c.from("payment_methods").select("name, status").order("name");
  if (error) {
...
export async function addPaymentMethod(name: string): Promise<string> {
  ...
  if (!trimmed) return "";
  const c = client();
  const { data: existing } = await c.from("payment_methods").select("name").ilike("name", trimmed).maybeSingle();
  if (existing) return (existing as { name: string }).name;
  const { data: inactive } = await c.from("payment_methods").select("id").ilike("name", trimmed).eq("status", "inactive").maybeSingle();
  if (inactive) {
...
export async function getPaymentMethodUsageCount(name: string): Promise<number> {
  const c = client();
  const { count, error } = await c.from("expenses").select("id", { count: "exact", head: true }).ilike("payment_method", name);
...
export async function renamePaymentMethod(oldName: string, newName: string): Promise<boolean> {
  const c = client();
  const { data: row } = await c.from("payment_methods").select("id").ilike("name", oldName.trim()).maybeSingle();
  if (!row) return false;
  await c.from("payment_methods").update({ name: newName.trim() }).eq("id", (row as { id: string }).id);
  const { error } = await c.from("expenses").update({ payment_method: newName.trim() }).ilike("payment_method", oldName.trim());
...
export async function disablePaymentMethod(name: string): Promise<boolean> {
  const c = client();
  const { error } = await c.from("payment_methods").update({ status: "inactive" }).ilike("name", name.trim());
  return !error;
}

export async function enablePaymentMethod(name: string): Promise<boolean> {
  const c = client();
  const { error } = await c.from("payment_methods").update({ status: "active" }).ilike("name", name.trim());
  return !error;
}
...
export async function isPaymentMethodDisabled(name: string): Promise<boolean> {
  const c = client();
  const { data: row } = await c.from("payment_methods").select("status").ilike("name", name.trim()).maybeSingle();
  return (row as { status: string } | null)?.status === "inactive";
}
