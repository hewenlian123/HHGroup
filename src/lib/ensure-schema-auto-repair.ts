...
$$`,
  ...
  `ALTER TABLE public.labor_entries ADD COLUMN IF NOT EXISTS days_worked numeric NULL`,
  `ALTER TABLE public.labor_entries ADD COLUMN IF NOT EXISTS daily_rate_snapshot numeric NULL`,
  `ALTER TABLE public.labor_entries ADD COLUMN IF NOT EXISTS amount_snapshot numeric NULL`,
  `ALTER TABLE public.labor_entries ADD COLUMN IF NOT EXISTS labor_cost_snapshot numeric NULL`,
  `ALTER TABLE public.labor_entries ADD COLUMN IF NOT EXISTS rate_history_id uuid NULL REFERENCES public.worker_rate_history(id) ON DELETE SET NULL`,
  ...
  `WITH earliest_labor AS (
   ...
   SELECT
     w.id,
     'daily',
     GREATEST(COALESCE(NULLIF(w.daily_rate, 0), NULLIF(w.half_day_rate, 0), 0), 0),
...
END $$`,
  ...
  `CREATE POLICY worker_rate_history_authenticated_select
   ON public.worker_rate_history FOR SELECT TO authenticated USING (true)`,
  `CREATE POLICY worker_rate_history_authenticated_insert
   ON public.worker_rate_history FOR INSERT TO authenticated WITH CHECK (true)`,
...
$$`,
  ...
  `REVOKE SELECT, INSERT, UPDATE, DELETE ON TABLE public.bank_transactions FROM anon`,
