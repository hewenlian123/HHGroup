...
export type GenerateMaterialSelectionPrintPdfOptions = {
  selectionId: string;
  origin: string;
  cookieHeader?: string | null;
};

const MATERIAL_SELECTION_DOCUMENT_SELECTOR = '[data-testid="material-selection-document"]';
...
export async function generateMaterialSelectionPrintPdfBuffer(
  options: GenerateMaterialSelectionPrintPdfOptions
): Promise<Buffer> {
  const { selectionId, origin, cookieHeader } = options;
  const base = origin.replace(/\/$/, "");
  const url = `${base}/materials/${encodeURIComponent(selectionId)}/print?pdf=1`;
  ...
  try {
    ...
    await page.waitForSelector(MATERIAL_SELECTION_DOCUMENT_SELECTOR, { timeout: 30_000 });
...
export function materialSelectionPrintPdfFilename(selectionNumber: string, title: string): string {
  const base = selectionNumber || title || "material-selection";
  const safe = base
    .replace(/[^\w.-]+/g, "_")
    .replace(/_+/g, "_")
    .replace(/^_|_$/g, "");
  return `Material-Selection-${safe || "draft"}.pdf`;
}
