...
const LABOR_DDL = [
  ...
  `ALTER TABLE public.worker_reimbursements ADD COLUMN IF NOT EXISTS vendor text`,
  `ALTER TABLE public.worker_reimbursements ADD COLUMN IF NOT EXISTS paid_at timestamptz`,
  ...
  `CREATE TABLE IF NOT EXISTS public.worker_reimbursement_payments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  worker_id uuid REFERENCES public.workers(id),
  ...
  `CREATE INDEX IF NOT EXISTS idx_worker_reimbursement_payments_worker_id ON public.worker_reimbursement_payments (worker_id)`,
  ...
  `DROP POLICY IF EXISTS worker_reimbursement_payments_select_all ON public.worker_reimbursement_payments`,
  `CREATE POLICY worker_reimbursement_payments_select_all ON public.worker_reimbursement_payments FOR SELECT TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS worker_reimbursement_payments_insert_all ON public.worker_reimbursement_payments`,
  `CREATE POLICY worker_reimbursement_payments_insert_all ON public.worker_reimbursement_payments FOR INSERT TO authenticated WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_reimbursement_payments_update_all ON public.worker_reimbursement_payments`,
  `CREATE POLICY worker_reimbursement_payments_update_all ON public.worker_reimbursement_payments FOR UPDATE TO authenticated USING (true) WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_reimbursement_payments_delete_all ON public.worker_reimbursement_payments`,
  `CREATE POLICY worker_reimbursement_payments_delete_all ON public.worker_reimbursement_payments FOR DELETE TO authenticated USING (true)`,
  `ALTER TABLE public.worker_payments ENABLE ROW LEVEL SECURITY`,
  `DROP POLICY IF EXISTS worker_payments_select_all ON public.worker_payments`,
  `CREATE POLICY worker_payments_select_all ON public.worker_payments FOR SELECT TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS worker_payments_insert_all ON public.worker_payments`,
  `CREATE POLICY worker_payments_insert_all ON public.worker_payments FOR INSERT TO authenticated WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_payments_update_all ON public.worker_payments`,
  `CREATE POLICY worker_payments_update_all ON public.worker_payments FOR UPDATE TO authenticated USING (true) WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_payments_delete_all ON public.worker_payments`,
  `CREATE POLICY worker_payments_delete_all ON public.worker_payments FOR DELETE TO authenticated USING (true)`,
  ...
  `CREATE POLICY daily_work_entries_select_all ON public.daily_work_entries FOR SELECT TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS daily_work_entries_insert_all ON public.daily_work_entries`,
  `CREATE POLICY daily_work_entries_insert_all ON public.daily_work_entries FOR INSERT TO authenticated WITH CHECK (true)`,
  `DROP POLICY IF EXISTS daily_work_entries_update_all ON public.daily_work_entries`,
  `CREATE POLICY daily_work_entries_update_all ON public.daily_work_entries FOR UPDATE TO authenticated USING (true) WITH CHECK (true)`,
  `DROP POLICY IF EXISTS daily_work_entries_delete_all ON public.daily_work_entries`,
  `CREATE POLICY daily_work_entries_delete_all ON public.daily_work_entries FOR DELETE TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS worker_reimbursements_select_all ON public.worker_reimbursements`,
  `CREATE POLICY worker_reimbursements_select_all ON public.worker_reimbursements FOR SELECT TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS worker_reimbursements_insert_all ON public.worker_reimbursements`,
  `CREATE POLICY worker_reimbursements_insert_all ON public.worker_reimbursements FOR INSERT TO authenticated WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_reimbursements_update_all ON public.worker_reimbursements`,
  `CREATE POLICY worker_reimbursements_update_all ON public.worker_reimbursements FOR UPDATE TO authenticated USING (true) WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_reimbursements_delete_all ON public.worker_reimbursements`,
  `CREATE POLICY worker_reimbursements_delete_all ON public.worker_reimbursements FOR DELETE TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS worker_invoices_select_all ON public.worker_invoices`,
  `CREATE POLICY worker_invoices_select_all ON public.worker_invoices FOR SELECT TO authenticated USING (true)`,
  `DROP POLICY IF EXISTS worker_invoices_insert_all ON public.worker_invoices`,
  `CREATE POLICY worker_invoices_insert_all ON public.worker_invoices FOR INSERT TO authenticated WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_invoices_update_all ON public.worker_invoices`,
  `CREATE POLICY worker_invoices_update_all ON public.worker_invoices FOR UPDATE TO authenticated USING (true) WITH CHECK (true)`,
  `DROP POLICY IF EXISTS worker_invoices_delete_all ON public.worker_invoices`,
  `CREATE POLICY worker_invoices_delete_all ON public.worker_invoices FOR DELETE TO authenticated USING (true)`,
];
