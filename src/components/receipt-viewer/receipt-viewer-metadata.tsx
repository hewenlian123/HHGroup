    ...
    <dl className="space-y-4">
      {rows.map((row) => (
        <div key={row.label} className="min-w-0">
          <dt className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--hh-text-tertiary)]">
            {row.label}
          </dt>
          <dd
            className={cn(
              "mt-1 break-words text-sm leading-5 text-[var(--hh-text-primary)]",
              row.label === "Amount" && "font-semibold tabular-nums text-[#d2b77f]"
            )}
          >
            {row.value}
          </dd>
...
export function ReceiptViewerMetadata({
    ...
    return (
      <div className="border-t border-white/10 bg-[rgb(24_27_30_/_0.96)]">
        <button
          ...
          className="flex min-h-11 w-full touch-manipulation items-center justify-between gap-3 px-4 py-2 text-sm font-medium text-[var(--hh-text-primary)] outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--hh-focus-ring)]"
        >
          <span>Receipt details</span>
          <ChevronDown
            className={cn(
              "h-4 w-4 text-[var(--hh-text-tertiary)] transition-transform duration-150 motion-reduce:transition-none",
  ...
  return (
    ...
    >
      <p className="mb-5 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--hh-text-tertiary)]">
        Receipt details
      </p>
      <MetadataRows rows={rows} />
    </aside>
