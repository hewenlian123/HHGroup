    ...
    <div className="receipt receipt-container">
      <div className="receipt-main">
        ...
        <div className="receipt-table-wrap receipt-no-break">
          ...
          <table className="receipt-table receipt-table--labor">
            <colgroup>
              <col className="receipt-col-date" />
              <col className="receipt-col-project" />
              <col className="receipt-col-session" />
            ...
            <tbody>
              {laborLines.length === 0 ? (
                ...
                laborLines.map((row) => (
                  <tr key={row.id}>
                    <td className="tabular-nums">{row.dateLabel ?? fmtDate(row.workDate)}</td>
                    <td>{row.projectName ?? "—"}</td>
                    <td>{row.sessionLabel ?? sessionReceiptLabel(row.session)}</td>
      ...
      <div className="receipt-bottom">
        ...
        <footer className="receipt-footer">
          <p>
            Retain for payroll, 1099, accounting, and records. {company.companyName} — worker
            payment receipt.
          </p>
        </footer>
      </div>
