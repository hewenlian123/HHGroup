-- VERIFIED PRODUCTION PRE-CUTOVER FIXTURE (2026-08-10, read-only catalog capture).
--
-- Local-Docker fixture only. It recreates the exact worker-receipt-related
-- policy/grant/bucket baseline observed in Production so the rollback can be
-- fingerprint-tested. Do not apply this file to Production: it intentionally
-- restores the historic public delivery and broad legacy policy model.
...
create policy phase3a_worker_receipts_anon_select
on public.worker_receipts
for select
to anon
using (true);

create policy phase3a_worker_receipts_authenticated_delete
on public.worker_receipts
for delete
to authenticated
using (true);
...
create policy phase3a_worker_receipts_authenticated_select
on public.worker_receipts
for select
to authenticated
using (true);

create policy phase3a_worker_receipts_authenticated_update
on public.worker_receipts
for update
to authenticated
using (true)
...
create policy worker_receipts_select
on public.worker_receipts
for select
to anon
using (true);
...
create policy phase3a_workers_anon_select
on public.workers
for select
to anon
using (true);

create policy phase3a_workers_authenticated_delete
on public.workers
for delete
to authenticated
using (true);

create policy phase3a_workers_authenticated_insert
on public.workers
for insert
to authenticated
with check (true);

create policy phase3a_workers_authenticated_select
on public.workers
for select
to authenticated
using (true);

create policy phase3a_workers_authenticated_update
on public.workers
for update
to authenticated
using (true)
...
create policy phase3a_projects_anon_select
on public.projects
for select
to anon
using (true);

create policy phase3a_projects_authenticated_delete
on public.projects
for delete
to authenticated
using (true);

create policy phase3a_projects_authenticated_insert
on public.projects
for insert
to authenticated
with check (true);

create policy phase3a_projects_authenticated_select
on public.projects
for select
to authenticated
using (true);

create policy phase3a_projects_authenticated_update
on public.projects
for update
to authenticated
using (true)
